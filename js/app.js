...
class GallerySystem {
  ...
  openModal(src) {
    ...
    const videoContainer = document.getElementById('gallery-modal-video-container');
    if (videoContainer) {
  ...
  openVideoModal(url) {
    ...
    if (videoContainer) {
      ...
      // Embed YouTube iframe with autoplay enabled
      videoContainer.innerHTML = `<iframe src="${url}?autoplay=1" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen style="width: 100%; height: 100%; border: none; border-radius: var(--radius-md);"></iframe>`;
    }
...
class VideoShowcase {
  constructor() {
    this.posters = document.querySelectorAll('.videos-grid .video-poster-overlay');
    this.init();
  }
  ...
  init() {
    ...
    this.posters.forEach(poster => {
      poster.addEventListener('click', () => {
        ...
        const container = poster.nextElementSibling;
        if (container && videoId) {
          poster.style.display = 'none';
          container.innerHTML = `<iframe src="https://www.youtube.com/embed/${videoId}?autoplay=1" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen style="width: 100%; height: 100%; border: none;"></iframe>`;
        }
      });
    });
  }
